...
if __name__=="__main__":
    ...
    stars = stars.drop_duplicates('EPIC')
    # Preprocess
    prepro = Preprocessing(pipeline, stars)
    #kics = prepro.preprocess(gaia='../Tables/k2_c2_dr2_4arcsec.csv')
    kics = prepro.preprocess(gaia='../Tables/k2_dr2_4arcsec.csv')
    print(kics)
    kmag = pd.read_csv('../Tables/APOK2_kmag.csv') #K2_c2_kmag.csv')

    print(np.shape(kics), np.shape(kmag))
    kics = pd.merge(kics, kmag, on=['EPIC'], how='inner')
    print(np.shape(kics))
    kics.dropna(axis=0, subset=['kmag', 'r_est'], inplace=True)
    print(np.shape(kics))
    extraction = FeatureExtractor(kics, pipeline=pipeline, NDAYS=NDAYS).extract
    ...
    kics = kics[pipeline['star_id']].values.astype(int)
