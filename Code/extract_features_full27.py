...
if __name__=="__main__":
    ...
    stars = stars.drop_duplicates('KIC')
    # Preprocess
    prepro = Preprocessing(pipeline, stars)
    kics = prepro.preprocess()
    extraction = FeatureExtractor(kics, NDAYS=NDAYS).extract
    ...
    print(kics.duplicated(subset='kepid', keep=False).sum()/len(kics))
        
    kics = kics['kepid'].values.astype(int)
