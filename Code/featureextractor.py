...
class FeatureExtractor():
    ...
    def extract(self, kicid, data_dir):
        ...
        if (self.pipeline['star_id'] == 'EPIC') and (self.pipeline['campaign'] != 'APOK2'):
            #thisfile = 
            #print(kicid, type(kicid), str(kicid)[:4], str(kicid)[:4].ljust(9, '0'))
            ...
            #plt.plot(star.new_time, star.new_flux)  
            #plt.show()

        else:
            #print(data_dir, kicid)
            try:
        ...
        #print(kicid)
        #print(self.prior_df.loc[self.prior_df['kepid'] == kicid,])
        #print(self.pipeline)
        #print(self.prior_df.head())
        prior_info = self.prior_df.loc[self.prior_df[self.pipeline['star_id']] == kicid,]

        # Extract additional information
       # print(list(prior_info))
...
class Preprocessing():
    ...
    def preprocess(self, gaia='../Tables/kepler_dr2_4arcsec.csv',
        ...
        if (self.pipeline['star_id'] == 'KIC') and (self.pipeline['data_type'] == 'APOKASC'):
            ...
            u, c = np.unique(kics, return_counts=True)
            #print("KICS: ", len(u), len(kics), len(u[c > 1])/len(kics))
